...
//            516701822D3051D1CE752F2969AE838C
    ...
    public ServerResponse<User> updateInformation(HttpSession session,User user) {
        User currentuUser = (User)session.getAttribute(Const.CURRENT_USER);
        if(currentuUser == null) {
            return ServerResponse.createByErrorMessage("用户未登录");
        }
        user.setId(currentuUser.getId());
        user.setUsername(currentuUser.getUsername());
        ServerResponse<User> response = iUserService.updateInformation(user);
        if(response.isSuccess()) {
            response.getData().setUsername(currentuUser.getUsername());
            session.setAttribute(Const.CURRENT_USER,response.getData());
        }
        return response;
    }
    ...
    public ServerResponse<User> getInformation(HttpSession session) {
        ...
        return iUserService.getInformation(currentuUser.getId());
    }
}